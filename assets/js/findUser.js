...
document.getElementById('send-btn').onclick = function () {
  if (confirm('Are you sure you want to do this? This will email every user in the system.')) {
    ...
    request.open('POST', '/users/points', true); // Set JSON header as well as the CSRF token header, both very important
...
document.getElementById('reset-btn').onclick = function () {
  if (confirm('Are you sure you want to reset all part timer point balances to zero?')) {
    var request = new XMLHttpRequest();
    request.open('POST', '/users/points/reset', true); // Set JSON header as well as the CSRF token header, both very important
