...
document.getElementById('email-btn').onclick = function() {
    if (originalUserName === 'testing@testing.com') {
        M.toast({html: 'Can\'t send points balance to a testing account.'});
        return;
    }
    if(confirm('Are you sure you want to email this user their point balance?')) {
        // Get id from url variable
        var id = url[url.length - 1];
        // Append id to correct url for this action
        var newURL = '/users/points/' + id;
        // Send request to server
        var request = new XMLHttpRequest();
        ...
        request.onload = function() {
            if (request.status >= 200 && request.status < 400) {
                ...
                M.toast({html: 'Failed to send email to user, please try again.'});
            }
        };
        request.send();
    }
